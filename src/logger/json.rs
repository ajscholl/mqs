...
mod test {
    ...
    fn log(logger: &Logger<TestWriter>, level: Level, message: &str, module_path: &'static str, file: &'static str, line: u32) {
        logger.log(
            &Record::builder()
    ...
    fn logger_test() {
        ...
        log(&logger, Level::Trace, "Should not appear", module_path!(), file!(), line!());
        log(&logger, Level::Debug, "This is also not needed", module_path!(), file!(), line!());
        log(&logger, Level::Info, "This should get logged", module_path!(), file!(), line!());
        log(&logger, Level::Warn, "And of course this", module_path!(), file!(), line!());
        log(&logger, Level::Error, "This has to get logged, otherwise would be bad", module_path!(), file!(), line!());

        logger.flush();

        logger.writer.lock().unwrap().get_mut().get_ref().assert_expectations(start_time, vec![
